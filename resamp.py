# LS 40 resampling library

### ACTIVE FINAL SCRIPT

## VERSION - 1.9
## DATE: 22 MAY 2026
## AUTHOR: VISHANTH HARI RAJ, JANE SHEVTSOV, TIRU VEDAM, ALEJANDRO MIRON JABALERA
## SUPERVISOR: JANE SHEVTSOV and KRISTIN MCCULLY
...
# find_power_numba by Tiru Vedam
import numpy as np 
from numba import njit, prange
@njit(parallel=True)
def stat(arr, measure_is_mean):
    if measure_is_mean:
        return arr.mean()
    else:
...
# @njit(parallel=True)
def null_distribution(pooled, sims, size1, size2, measure_is_mean):
    n = len(pooled)
    null_diffs = np.zeros(sims)
    for j in prange(sims):
        g1 = pooled[np.random.randint(0, n, size1)]
        g2 = pooled[np.random.randint(0, n, size2)]
        null_diffs[j] = stat(g1, measure_is_mean) - stat(g2, measure_is_mean)
    return null_diffs

# @njit
# def find_power_numba(group1, group2, sample_size1, sample_size2, sims=1000, alpha=0.01, measure='median', verbose=False):
#     group1 = np.asarray(group1)
# def find_power_numba(group1, group2, sample_size1, )
...
#     for j in prange(sims):                       # Parallel loop
#         g1 = pooled[np.random.randint(0, n, size1)]  
#         g2 = pooled[np.random.randint(0, n, size2)]  
#         null_diffs[j] = stat(g1, measure_is_mean) - stat(g2, measure_is_mean)
...
#wrapper that students call:
def find_power_numba(group1, group2, samp_size1, samp_size2,
                     sims=1000, alpha=0.01, measure='median', verbose=False):

    # Convert "median"/"mean" → boolean because Numba cannot use strings
    measure_is_mean = (measure == 'mean')

    # Convert to arrays (Numba needs contiguous ndarray inputs)
    group1 = np.asarray(group1)
    ...
    for i in range(sims):  
        sim_g1 = np.random.choice(group1, samp_size1, replace=True)
        sim_g2 = np.random.choice(group2, samp_size2, replace=True)

        phantom_diff = stat(sim_g1, measure_is_mean) - stat(sim_g2, measure_is_mean)

        pooled = np.concatenate([sim_g1, sim_g2])

        null_diffs = null_distribution(pooled, sims, samp_size1, samp_size2, measure_is_mean)

        pvals[i] = np.mean(np.abs(null_diffs) >= abs(phantom_diff))
